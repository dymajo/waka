...
const log = require('../../server-common/logger')
...
const connectMaster = async function() {
  ...
  try {
    ...
    await pool.request()
      .query(`If(db_id(N'${config.database}') IS NULL) CREATE DATABASE ${config.database}`)
  } catch(err) {
    console.error('master'.red, 'Failed to connect to master database! Check settings in config-master.js'.red)
    console.error(err)
