...
class VehicleLocationBootstrap extends React.Component {
  ...
  componentDidUpdate() {
    if (typeof this.state.tripInfo.route_long_name !== 'undefined') {
      if ('line_id' in this.props.match.params) {
        ...
        document.title = t('vech_loc.header', {
          route: this.state.tripInfo.route_short_name,
          appname: t('app.name'),
        })
      }
    }
  ...
  getLineData = () => {
    fetch(
      `${local.endpoint}/${this.props.match.params.region}/line/${
  ...
  render() {
    ...
    if (
      ...
      (this.state.animation !== 'entering' ||
        this.state.animation !== 'unmounted')
    ) {
      if ('line_id' in this.props.match.params) {
          ...
          lineSelect = this.state.lineInfo.map(function(line, key) {
            return (
              <option key={key} value={key}>
                {line.route_long_name.replace(/ Train Station/g, '')}
              </option>
            )
          })
          lineSelect = (
            <select onChange={this.triggerChange}>{lineSelect}</select>
          )
        }
      } else {
        let stopInfo = [
          this.state.stopInfo.stop_lat,
          this.state.stopInfo.stop_lon,
        ]
    ...
    return (
      <div
        className="vehicle-location-container"
            ...
            <h1 className="line-name">
              <section className="line-pill-wrapper-header">
                <span
                  ...
                  style={{ backgroundColor: this.state.tripInfo.route_color }}
                >
                  {code}
                </span>
              </section>
