...
export class LineData {
  ...
  getShape() {
    return new Promise((resolve, reject) => {
      if (this.region === null || this.shape_id === null) {
        return reject(new Error('Requires both region and shape_id to be set.'))
      }
  ...
  getStops() {
    return new Promise((resolve, reject) => {
      ...
      if (this.trip_id !== null) {
        ...
        return reject(new Error('Requires shape_id or trip_id to be set.'))
      }
      fetch(url)
        .then(response => response.json())
        .then(data => {
  ...
  getRealtime() {
    return fetch(`${local.endpoint}/${this.region}/realtime/${this.line_id}`)
      .then(r => {
        if (r.status >= 400) {
