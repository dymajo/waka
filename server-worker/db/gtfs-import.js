...
class gtfsImport {
  unzip(location) {
      ...
      const locations = location.map(
        loc =>
          new Promise((resolve, reject) => {
            ...
            extract(loc, { dir: path.resolve(`${loc}unarchived`) }, err => {
              if (err) {
                return reject(err)
              }
              log('Unzip Success!')
    ...
    return new Promise((resolve, reject) => {
      log('Unzipping GTFS Data')
      extract(location, { dir: path.resolve(`${location}unarchived`) }, err => {
        if (err) {
          return reject(err)
        }
        log('Unzip Success!')
    ...
    return new Promise((resolve, reject) => {
      ...
      const transformer = transform(
        (row, callback) => {
          ...
          const processRow = () => {
            if (row && row.length > 1) {
              const record = this._mapRowToRecord(
                row,
                headers,
                schemas[config.table]
              )
